...
exports.${name}Response = (err, data, res) => {
  if(err) {
    logger.error({code:"#${name}${ctrlName.capitalize()}Error",error:err,input:req.body})
    errors = [{
...
exports.generateModel = (directory, ctrlName) => {
  directory = directory.concat(`/models/${ctrlName}.res.js`);
...
exports.generateRoute = (path, name, ctrlName) => {
  ...
  let importString = `
var ${ctrlName}Request = require('../routes/${ctrlName}.req.js');
`
  let routeString = `
router.${url[name]}, (req, res) => { ${ctrlName}Request.${name}Request('${ctrlName}', '${name}${ctrlName.capitalize()}', req, res) });
`
...
var router = express.Router();${importString}${routeString}\nmodule.exports = router;`
      fs.writeFile(directory, request, function (err) {
        ...
        directory = path.concat(`/routes/index.js`);
...
exports.generateFrameWork = (directory, name) => {
  fs.rmdir(`${directory}/${name}`, function(err) {
    if(err && err.errno != -2) {
      ...
      shell.cd(directory);
      shell.exec('git clone https://github.com/mrafiqk/node-frame.git');
      fs.rename(`${directory}/node-frame`, `${directory}/${name}`, function(err) {
        if (err) {
          console.log(err)
        } else {
...
exports.${name}${ctrlName.capitalize()} = (req,res) => {
  ...
  ${ctrlName.capitalize()}.findOneAndUpdate(query, { s: "D" }, (err, docs)=>{
    ${ctrlName}Response.${name}Response(err, docs, res)
  })
}
`,
