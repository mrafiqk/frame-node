...
exports.generateRequest = (directory, name, ctrlName) => {
  ...
  let requestStr = `
exports.${name ? name : 'get'}Request = (role, controller, method_name, req, res) => {
...
exports.generateResponse = (directory, name, ctrlName) => {
  ...
  let requestStr = `
exports.${name}Response = (err, data, input, res) => {
  if(err) {
    logger.error({ code: "#${name}${ctrlName.capitalize()}Error", error:err, input: input })
    errors = [{
...
exports.generateRoute = (path, name, ctrlName) => {
  ...
  let importString = `
var ${ctrlName}Request = require('../requests/${ctrlName}.req');
var ${ctrlName}Ctrl = require('../requests/${ctrlName}.ctrl');
`
  let routeString = `
router.${url[name] ? url[name] : 'get(\'/' + name +'\''}, (req, res) => { ${ctrlName}Request.${name}Request(['all'], ${ctrlName}Ctrl, '${name}${ctrlName.capitalize()}', req, res) });
`
...
exports.${name}${ctrlName.capitalize()} = (req,res) => {
  ...
  ${ctrlName.capitalize()}.findOne(query, (err, docs)=>{
    ${ctrlName}Response.${name}Response(err, docs, query, res);
  })
}
`,
    search: `
